...
Los usuarios son los que distribuyen sus claves públicas. Esto puede generar problemas de confianza, ya que no hay una entidad que verifique la autenticidad de las claves, cualquiera puede hacerse pasar por otra persona.
...
- Funciones:
  ...
  - Usa el protocolo OCSP(Online Certificate Status Protocolo) para la validación, aunque no está en la arquitectura PKI.
...
- Certificación: Certifica que una clave pública pertenece a una entidad final, devolviendo el certificado al cliente o almacenandolo en el repositorio.
...
- Recuperación del Par de Claves: Permite que la entidades finales recuperen sus claves a través de una entidad autorizada que las resguarda. Normalmente, es la CA que emitió el certificado. Sin embargo, si se pierde la clave privada, la entidad final debe generar un nuevo par de claves y obtener un nuevo certificado. **Esto no es recomendable desde el punto de vista de seguridad.**
...
<div style="text-align:center;">
  <img src="image-24.png" alt="OCSP Diagram"  width="400"/>
</div>
...
sobre digital es un método de ciberseguridad que permite enviar un mensaje de forma segura combinando lo mejor de dos tipos de cifrado: la velocidad del cifrado simétrico y la seguridad del cifrado asimétrico (de clave pública).

Piénsalo de esta manera:

El cifrado simétrico (que usa una sola clave compartida) es como una caja fuerte de alta velocidad. Es muy rápido para bloquear y desbloquear, pero tienes el arriesgado problema de cómo entregarle esa clave compartida a tu destinatario de forma segura.

El cifrado asimétrico (que usa un par de claves pública/privada) es como un buzón súper seguro. Cualquiera puede dejar un mensaje usando la ranura pública (clave pública), pero solo tú, con la llave privada única, puedes abrirlo. Este proceso es muy seguro, pero también muy lento.

Un sobre digital usa el buzón seguro (asimétrico) para enviar de forma protegida la llave de la caja fuerte de alta velocidad (simétrica).
...
Este es el proceso para que un remitente (digamos Alicia) envíe un mensaje seguro a un destinatario (Beto).

1. **El lado del remitente (Creando el sobre)**

- Cifrar el mensaje: Alicia quiere enviar un documento grande. Primero, genera una clave simétrica completamente nueva y de un solo uso (también llamada "clave de sesión"). Utiliza esta clave rápida para cifrar su documento completo.

- Cifrar la clave: Ahora, Alicia necesita enviar esta clave simétrica a Beto de forma segura. Toma la clave pública de Beto (que Beto comparte con todo el mundo) y la usa para cifrar la pequeña clave simétrica que acaba de crear.

- Empaquetar y enviar: Alicia le envía dos cosas a Beto:

- El documento grande cifrado (ilegible para cualquiera).

- La clave simétrica cifrada (este es el "sobre digital").

2. **El lado del destinatario (Abriendo el sobre)**

- Recibir el paquete: Beto recibe el documento grande cifrado y el pequeño sobre digital.

- Desbloquear el sobre: Beto usa su propia clave privada (que mantiene en secreto y nunca comparte) para descifrar el sobre digital. Esto "desbloquea" el sobre y revela la clave simétrica original que creó Alicia.

- Descifrar el mensaje: Beto ahora tiene la clave simétrica. Utiliza esta clave rápida para descifrar velozmente el documento grande, revelando el mensaje original en texto plano.
